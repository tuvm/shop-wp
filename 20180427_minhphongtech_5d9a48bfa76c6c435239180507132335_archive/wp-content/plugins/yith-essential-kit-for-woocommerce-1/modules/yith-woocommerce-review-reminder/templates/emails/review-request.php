...
$unsubscribe_link    = ( array_key_exists( $template, YITH_WRR()->_email_templates ) || defined( 'YITH_WCET_PREMIUM' ) && get_option( 'ywrr_mail_template_enable' ) == 'yes' ) ? '' : '<a href="' . $unsubscribe_url . '">' . $unsubscribe_text . '</a>';
...
if ( $email_templates || ( ! $template ) || ( $template == 'base' ) ) {
	...
	do_action( 'woocommerce_email_footer', $email, array( '<a class="ywrr-unsubscribe-link" href="' . $unsubscribe_url . '">' . $unsubscribe_text . '</a>' ) );
