...
<div id="s1-area-setup-type" style="display:none">
	...
	<label for="setup-type-fresh"><b>Fresh Install</b></label>
	<i class="fa fa-question-circle"
		data-tooltip-title="Fresh Install"
		data-tooltip="A fresh install is the default way Duplicator has always worked.  Setup your package in an empty directory and run the installer."></i>
	<br/>
	...
	<!-- IMPORT OVERWRITE -->
	<input type="radio" id="setup-type-import" name="setup_type" value="2" onclick="DUPX.toggleSetupType()" />
	<label for="setup-type-import"><b>Import Overwrite</b></label>
	<i class="fa fa-question-circle"
		data-tooltip-title="Import Overwrite"
		data-tooltip="An import overwrite allows Duplicator Pro to import a WordPress site over an existing WordPress Site."></i><br/>
	<div class="s1-setup-type-sub" id="s1-setup-type-sub-2">
		<input type="checkbox" name="setup-backup-files" id="setup-backup-files-import" />
		<label for="setup-backup-files-import">Backup Existing Files</label><br/>
		<input type="checkbox" name="setup-remove-files" id="setup-remove-files-import" />
		<label for="setup-remove-files-import">Remove Existing Files</label><br/>
		<input type="checkbox" name="setup-backup-database" id="setup-backup-database-import" />
		<label for="setup-backup-database-import">Backup Existing Database</label> <br/>
	</div><br/>
	...
	<div class="s1-reqs" id="s1-notice-all">
		...
		<?php if (!$GLOBALS['DUPX_AC']->exportOnlyDB) :?>
			...
			<div class="info" id="s1-notice01">
				...
				directory it might indicate that a pre-existing WordPress site exists which can lead to a bad install.  <i>If this archive was manually extracted then
        ...
        <tr>
            ...
            <td>
                ...
                <select id="archive_engine" name="archive_engine" size="<?php echo $num_selections; ?>">
                    <?php if($installer_state->mode !== DUPX_InstallerMode::SiteOverwrite) : ?>
...
DUPX.processNext = function ()
{
   ...
   var moveCoreFiles = <?php DUPX_U::echoBool($installer_state->mode === DUPX_InstallerMode::SiteOverwrite); ?>;
