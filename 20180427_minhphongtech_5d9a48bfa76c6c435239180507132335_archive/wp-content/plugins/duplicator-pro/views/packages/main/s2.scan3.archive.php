...
defined("ABSPATH") or die("");
	...
	$dbbuild_mode = ($mysqlDumpPath && $global->package_mysqldump) ? "mysqldump" : 'PHP';
	...
	<div class="info" id="scan-item-file-size">
		...
		<div id="size-more-details">
			<?php
				...
				printf(DUP_PRO_U::__('This notice is triggered at <b>%s</b> and can be ignored on most hosts.  If during the build process you see a "Host Build Interrupt" '
					. 'message then this host has strict processing limits.  Below are some options you can take to overcome constraints setup on this host.'),
...
jQuery(document).ready(function($)
{
	...
	DupPro.Pack.copyText = function(btn, query)
	{
		$(query).select();
		 try {
		   ...
		   alert("<?php DUP_PRO_U::_e('Manual copy of selected text required on this browser.');?>")
		 }
	}
	...
	DupPro.Pack.applyFilters = function(btn, type)
	{
		...
		$.ajax({
			...
			error: function(data) {
				console.log(data);
				alert("<?php DUP_PRO_U::_e('Error applying filters.  Please go back to Step 1 to add filter manually!');?>");
			}
		});
	}
