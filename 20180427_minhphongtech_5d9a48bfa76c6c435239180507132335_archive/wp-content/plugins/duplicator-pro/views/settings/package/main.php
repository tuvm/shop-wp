...
$spacer = ' &nbsp;|&nbsp; ';
$show_brd = $global->profile_idea ? 'style="display:inline-block"' : 'style="display:none"';
...
switch ($section) {
	...
	case 'basic':
		...
		<div class='dpro-sub-tabs'>
			...
			<span {$show_brd}>{$spacer}<a href='{$url}&sub=brand'>{$txt_brd}</a></span>
		</div>";
	...
	case 'brand':
		...
		<div class='dpro-sub-tabs'>
			<a href='{$url}&sub=basic'>{$txt_gen}</a>{$spacer}
			<a href='{$url}&sub=advanced'>{$txt_adv}</a>
			<span {$show_brd}>{$spacer}<b>{$txt_brd}</b></span>
		</div>";
